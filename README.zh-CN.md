...
- **完整交互**：主画布聚焦后可用 `↑/↓` 或 Page Up/Down 纵向浏览、`←/→` 平移时间窗口、`Home` 查看整图、`End` 切换到 52 秒细看刻度；同时支持滚轮、`Ctrl/⌘+滚轮` 缩放、拖拽平移、过滤、搜索、详情面板、SVG/PNG 导出和明暗主题。
- **正反缩放**：`Fit` 展示完整轨迹，反向的 `52 秒` 按钮以选中步骤为中心，把未被详情面板遮挡的可见时间范围对齐到约 52 秒。
...
- 初始浏览器 payload 不包含完整工具结果；只有用户展开某个结果时才通过 opaque ID 接口读取未截断全文。思考摘要仍限制为 2000 字。
