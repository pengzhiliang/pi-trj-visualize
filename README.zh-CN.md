...
- **工具级失败**：只要 tool-use message 同时有模型说明，即使唯一的工具失败，整个 assistant Step 仍留在绿色主干；失败工具单独显示红色细条。
...
- **完整交互**：主画布聚焦后可用 `↑/↓` 或 Page Up/Down 纵向浏览、`←/→` 平移时间窗口、`Home` 查看整图、`End` 切换到 60 秒细看刻度；同时支持滚轮、`Ctrl/⌘+滚轮` 缩放、拖拽平移、过滤、搜索、详情面板、SVG/PNG 导出和明暗主题。
- **正反缩放**：`Fit` 展示完整轨迹，反向的 `60 秒` 按钮以选中步骤为中心，把未被详情面板遮挡的可见时间范围对齐到约 60 秒。
- **灵活标签**：高缩放下重复工具名自动计数压缩，例如 `bash · read ×4`；每次调用仍分别显示在工具瀑布中。
...
- **图片结果**：Pi session 中 user/tool result 的 `image` block 会在详情面板按需加载，支持点击展开原图；初始 payload 不再复制体积很大的 base64 图片数据。
...
- **胶囊下方细条**：同一步里的并行 tool call 和单个失败调用，每条使用自己的起止时间和结果判定色。
...
点击节点可查看用户提示、模型回答、思考摘要、模型/stop reason、token、工具参数和判定依据；工具结果默认折叠，展开后加载完整未截断内容。
...
- 初始浏览器 payload 不包含完整工具结果或 base64 图片数据；只有用户展开结果或查看图片时才通过 opaque ID 接口按需读取。思考摘要仍限制为 2000 字。
- 主会话和各 Subagent 的 JSONL 读取会并发启动；重复详情请求共享同一个进行中的解析任务，列表到详情的紧邻请求复用最新 session 索引。
