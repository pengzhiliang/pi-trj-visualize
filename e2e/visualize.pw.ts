...
test('browses inline Subagents, inspects, scrolls panels, and themes Pi sessions', async ({ page }) => {
  ...
  await expect(maze.locator('#legend .lane-key')).toHaveCount(2)
  await expect(maze.locator('#legend .lane-key').nth(0)).toHaveText('Main')
  await expect(maze.locator('#legend .lane-key').nth(1)).toHaveText('Subagent ×1')
  await expect(maze.locator('#legend')).not.toContainText('Explore#child123')
  ...
  await expect(imageResult.locator('.pimages img')).toHaveAttribute('src', /^\/api\/image\?/)
  await expect.poll(() => imageResult.locator('.pimages img').evaluate(image => image.naturalWidth)).toBeGreaterThan(0)
  ...
  await expect(maze.locator('#btnDetail')).toHaveText('↔ 60s')
  ...
  expect(detailWindow.visibleSpan).toBeCloseTo(60, 0)
  ...
  await expect(maze.locator('#btnDetail')).toHaveText('↔ 60 秒')
  await page.locator('#langToggle').click()
  await expect(page.locator('html')).toHaveAttribute('lang', 'en')
